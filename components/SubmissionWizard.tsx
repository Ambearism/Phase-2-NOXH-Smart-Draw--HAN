...
export function SubmissionWizard({ currentUser, onGoBack, onSubmit }: SubmissionWizardProps) {
  ...
  const [form02Data, setForm02Data] = useState(currentUser.form02Data || {
    ...
    familyMembers: [{ id: Math.random().toString(36).substr(2, 9), name: '', relationship: '', cccd09: '', cccd12: '' }],
    housingStatus: {
      noHome: false,
      lowArea: false,
      farAway: false,
  ...
  const isForm02Valid = form02Data.registrationRight && form02Data.currentAddress && form02Data.contactPhone && form02Data.familyMembers.every(m => m.name && (m.cccd09 || m.cccd12));
      ...
      <div className="p-6 overflow-y-auto flex-1 pb-24">
        ...
        {step === 1 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  ...
                  <button onClick={addFamilyMember} className="w-8 h-8 rounded-full bg-blue-100 text-[#00468E] flex items-center justify-center font-bold font-mono shadow-sm hover:bg-blue-200 transition-colors">+</button>
                </div>
                <div className="space-y-3">
                  {form02Data.familyMembers.map((member, idx) => (
                    <div key={member.id} className="bg-white p-4 rounded-2xl border border-slate-100 shadow-sm relative animate-fade-in">
                      <div className="flex items-center gap-2 mb-3">
                        ...
                        />
                        {idx >= 0 && (
                          <button onClick={() => removeFamilyMember(member.id)} className="text-[10px] font-black text-red-500 uppercase px-2 py-1 bg-red-50 rounded-lg">Xóa</button>
                        )}
                      </div>
                      <div className="grid grid-cols-1 gap-4">
                        <div className="space-y-1">
            ...
            <button 
              disabled={!isForm02Valid}
              onClick={() => setStep(2)}
              className="w-full mt-6 py-4 rounded-2xl font-black uppercase tracking-wider transition-all bg-[#00468E] text-white disabled:opacity-50 shadow-lg shadow-blue-100"
            >
              Tiếp Theo (Bước 2/6)
            </button>
          </div>
        ...
        {step === 2 && (
          <div className="space-y-4 animate-fade-in">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setStep(1)} className="w-1/3 py-4 rounded-2xl font-black uppercase text-slate-500 bg-slate-200">Quay Lại</button>
              <button 
                disabled={!selectedGroup}
                onClick={() => setStep(3)}
                className="w-2/3 py-4 rounded-2xl font-black uppercase tracking-wider transition-all bg-[#00468E] text-white disabled:opacity-50"
              >
                Tiếp Tục
              </button>
            </div>
        ...
        {step === 5 && (
          <div className="space-y-6 animate-fade-in">
            ...
            <div className="flex gap-3 mt-6">
              <button onClick={() => setStep(4)} className="w-1/3 py-4 rounded-2xl font-black text-slate-500 bg-slate-200 uppercase text-xs">Quay Lại</button>
              <button
